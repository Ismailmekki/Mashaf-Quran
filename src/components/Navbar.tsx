...
import { BookOpen, Search, Sun, Moon, Palette, Type, Sparkles, Smartphone, Volume2 } from "lucide-react";
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-1.5 sm:top-2 z-40 w-full backdrop-blur-md bg-[#12161F]/90 border-b border-[#2D3748] shadow-sm">
      <div className="max-w-7xl mx-auto px-3 sm:px-8 h-16 flex items-center justify-between gap-3">
        ...
        >
          ...
          <div className="w-10 h-10 border border-[#C5A059] rotate-45 flex items-center justify-center bg-[#1A202C] group-hover:border-[#E2C785] transition-colors shrink-0 shadow-sm">
            <BookOpen className="-rotate-45 w-4 h-4 text-[#C5A059]" />
          </div>
          <div>
            <span className="text-[#C5A059] font-medium tracking-[0.2em] text-[10px] uppercase block leading-tight">
              The Noble Quran
            </span>
            <div className="flex items-center gap-1.5">
              <h1 className="font-bold text-base sm:text-lg text-white font-tajawal tracking-tight leading-tight">
                القرآن الكريم
              </h1>
              <span className="text-[9px] px-1.5 py-0.2 uppercase tracking-wider font-mono bg-[#1A202C] text-[#C5A059] border border-[#8B6E3D]/50">
                APK
              </span>
            </div>
          </div>
        ...
        {isPlayingAudio && (
          <div
            ...
            className="flex items-center gap-1.5 px-3 py-1 bg-[#1A202C] border border-[#C5A059] text-[#C5A059] text-xs cursor-pointer hover:bg-[#2D3748] transition-colors animate-pulse shadow-sm"
          >
            <Volume2 className="w-3.5 h-3.5 text-[#C5A059]" />
            <span className="truncate max-w-[100px] sm:max-w-[150px] font-tajawal font-medium">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className="p-2 sm:px-3 sm:py-1.5 bg-[#1A202C] hover:bg-[#2D3748] text-stone-200 hover:text-white border border-[#2D3748] hover:border-[#C5A059] flex items-center gap-1.5 transition-colors cursor-pointer text-xs"
          >
          ...
          <button
            ...
            className={`p-2 sm:px-3 sm:py-1.5 border flex items-center gap-1.5 transition-colors cursor-pointer text-xs ${
              activeTab === "tadabbur"
          ...
          <button
            ...
            className="p-2 sm:px-3 sm:py-1.5 bg-[#C5A059] hover:bg-[#B38F46] text-[#1A202C] font-bold text-xs uppercase tracking-wider flex items-center gap-1.5 transition-all cursor-pointer shadow-sm border border-[#8B6E3D]"
          >
